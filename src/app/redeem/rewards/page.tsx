...
import {
  useGetMyLoyaltyHistory,
  useGetMyTotalLoyalties,
} from "@/services/queries";
import { WalletIcon, ActivityIcon } from "lucide-react";
import { FaGift } from "react-icons/fa";

const Rewards = () => {
  const { data: myTotalLoyalties } = useGetMyTotalLoyalties();
  const { data: myLoyaltyHistory } = useGetMyLoyaltyHistory();
  const points = (myTotalLoyalties && myTotalLoyalties.points) || 0;
  console.log("total", myTotalLoyalties);
  console.log("history", myLoyaltyHistory);
  return (
    <>
      {myTotalLoyalties !== undefined && myLoyaltyHistory !== undefined && (
        <Card className="relative border-0 p-3">
          ...
          <CardContent>
            <p className="mb-4 text-center text-lg text-muted-foreground">
              Manage your accumulated loyalty points and redeem them for
              exciting rewards.
            </p>
            ...
            <div className="flex flex-col items-center justify-between gap-6">
              ...
              <div className="flex items-center justify-between gap-6">
                ...
                <PointsHistory data={myLoyaltyHistory} />
              </div>
            </div>
          </CardContent>
          <CardFooter>
